...
import type { Camera, Lens, FilmStock, Roll, FilmHolder, Photograph } from "../api/client";
...
function toFormState(p: Photograph) {
  return {
    ...
    taken_at: p.taken_at ? p.taken_at.slice(0, 16) : "",
    aperture: p.aperture ?? "",
    ...
    altitude_m: p.altitude_m != null ? String(p.altitude_m) : "",
    gps_accuracy_m: p.gps_accuracy_m != null ? String(p.gps_accuracy_m) : "",
    notes: p.notes ?? "",
  };
}
...
export function PhotoEditPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([photo]) => setForm(toFormState(photo)));
  }, [id]);
    ...
    try {
      ...
      for (const [k, v] of Object.entries(form)) {
        if (v === "") {
          ...
          payload[k] = NUM_FIELDS.has(k) ? parseFloat(v) : v;
        }
      }
      await api.updatePhotograph(id, payload);
  ...
  return (
    <div className="page page-narrow">
      <div className="page-header">
        ...
        <Link to={`/app/photos/${id}`} className="link-btn">Cancel</Link>
      </div>
